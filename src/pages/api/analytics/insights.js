...
export default async function handler(req, res) {
  ...
  auth(req, res, async () => {
    try {
      ...
      const ordersWithDocsStages = organisation
        ? currentMatchStage
        : [{ $match: baseMatch }];
      ...
      if (salesGrowth > 10) {
        insights.push({
          type: "positive",
          message: `Sales increased by ${salesGrowth.toFixed(
            1
        ...
        insights.push({
          type: "negative",
          message: `Sales declined by ${Math.abs(salesGrowth).toFixed(
            1
          )}% compared to previous period`,
      ...
      if (current.profitMargin < thresholds.minProfitMargin) {
        insights.push({
          type: "warning",
          message: `Profit margin (${current.profitMargin.toFixed(
            1
          )}%) is below target of ${thresholds.minProfitMargin}%`,
        ...
        insights.push({
          type: "improvement",
          message: `Profit margin improved by ${marginChange.toFixed(
            1
          )} percentage points`,
          value: `${current.profitMargin.toFixed(1)}%`,
        });
      ...
      if (current.expenseRatio > thresholds.maxExpenseRatio) {
        insights.push({
          type: "warning",
          message: `Expense ratio (${current.expenseRatio.toFixed(
            1
          )}%) exceeds target of ${thresholds.maxExpenseRatio}%`,
          action:
      ...
      if (ordersGrowth > 15) {
        insights.push({
          type: "positive",
          message: `Order volume increased by ${ordersGrowth.toFixed(
            1
          )}% - strong demand trend`,
        ...
        insights.push({
          type: "negative",
          message: `Order volume decreased by ${Math.abs(ordersGrowth).toFixed(
            1
          )}%`,
      ...
      if (lrCompletionRate < thresholds.minDocumentCompletion) {
        insights.push({
          type: "warning",
          message: `Only ${lrCompletionRate.toFixed(
            0
      ...
      if (invoiceCompletionRate < thresholds.minDocumentCompletion) {
        insights.push({
          type: "warning",
          message: `Only ${invoiceCompletionRate.toFixed(
            0
          )}% of orders are invoiced`,
      ...
      if (cancellationRate > 5 && current.cancelled > 2) {
        insights.push({
          ...
          message: `${
            current.cancelled
          } orders cancelled (${cancellationRate.toFixed(
            1
          )}% cancellation rate)`,
      ...
      if (targets.sales && current.sales >= targets.sales) {
        ...
        insights.push({
          type: "warning",
          message: `Sales at ${achievement.toFixed(
            0
          )}% of target - significant gap remains`,
      ...
      if (insights.length === 0) {
        insights.push({
          type: "info",
          message:
            "Operations are stable with no significant anomalies detected",
          value: "All good",
        });
      }
