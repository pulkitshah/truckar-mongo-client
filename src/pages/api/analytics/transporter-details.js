...
export default async function handler(req, res) {
  ...
  auth(req, res, async () => {
    try {
      ...
      const { account, transporterId, startDate, endDate } = req.query;

      if (!account || !transporterId) {
        return res
          .status(400)
          .json({ message: "Account and transporterId are required" });
      }
      ...
      const orders = await Order.aggregate([
        ...
        },
        {
          $addFields: {
            ...
            transporterName: { $arrayElemAt: ["$transporterDetails.name", 0] },
            totalQuantity: {
              $sum: {
                $map: {
                  input: "$deliveries",
