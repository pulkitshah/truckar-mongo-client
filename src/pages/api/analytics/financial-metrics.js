...
export default async function handler(req, res) {
  ...
  auth(req, res, async () => {
    try {
      ...
      const marginTrend = trendData.map((d) => Number.parseFloat(d.margin.toFixed(2)));
      const aovTrend = trendData.map((d) => Math.round(d.aov));
      const expenseRatioTrend = trendData.map((d) => Number.parseFloat(d.expenseRatio.toFixed(2)));
