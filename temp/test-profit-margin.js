...
async function testProfitMarginCalculation() {
  try {
    ...
    const current = currentData[0] || { totalSales: 0, totalCost: 0, totalOrders: 0 };
    
    const profit = current.totalSales - current.totalCost;
    const profitMargin = current.totalSales > 0 ? (profit / current.totalSales) * 100 : 0;
    const expenseRatio = current.totalSales > 0 ? (current.totalCost / current.totalSales) * 100 : 0;
    ...
    if (profitMargin < 15) {
      console.log(`⚠️  Profit margin (${profitMargin.toFixed(1)}%) is below target of 15%`);
      console.log('   → Review operational costs and pricing structure');
    } else {
      console.log(`✅ Profit margin (${profitMargin.toFixed(1)}%) is above target of 15%`);
    }
