...
async function testAllDocumentCompletionLogic() {
  try {
    ...
    const lrCompletionRate = docStats.totalOrders > 0
      ? (docStats.ordersWithLR / docStats.totalOrders) * 100
      : 0;
    const invoiceCompletionRate = docStats.totalOrders > 0
      ? (docStats.ordersWithInvoice / docStats.totalOrders) * 100
      : 0;
    const fullCompletionRate = docStats.totalOrders > 0
    ...
    console.log('  Invoice Completion:', invoiceCompletionRate.toFixed(2) + '%');
    console.log('  Full Completion (Both):', fullCompletionRate.toFixed(2) + '%');
    ...
    console.log(`  Subtitle: ${docStats.totalOrders - docStats.ordersWithLR} pending LRs, ${docStats.totalOrders - docStats.ordersWithInvoice} pending invoices`);
    
    console.log('\nKEY INSIGHTS:');
    if (lrCompletionRate < 80) {
      console.log(`  ⚠️  Only ${lrCompletionRate.toFixed(0)}% of orders have LRs generated`);
    } else {
      console.log(`  ✅ ${lrCompletionRate.toFixed(0)}% of orders have LRs generated (above 80% threshold)`);
    }
    
    if (invoiceCompletionRate < 80) {
      console.log(`  ⚠️  Only ${invoiceCompletionRate.toFixed(0)}% of orders are invoiced`);
    } else {
      console.log(`  ✅ ${invoiceCompletionRate.toFixed(0)}% of orders are invoiced (above 80% threshold)`);
    }
